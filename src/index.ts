...
// Data validation
const { error, success, data } = srcDataSchema.safeParse(jsonData);
...
if (!success) {
  ...
  const revenueSum = revenues.reduce((acc, curr) => acc + curr.total_value, 0);
  console.log("Revenue: ", revenueSum);
  // Expenses
  const expenses = datas.filter(
    (data) => data.account_category === AccountCategory.EXPENSE
  );
  const expensesSum = expenses.reduce((acc, curr) => acc + curr.total_value, 0);
