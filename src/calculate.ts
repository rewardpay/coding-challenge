...
export function Revenue(data:any) {
    ...
    const sum = items.reduce((accumulator: number, currentItem: any) => 
        accumulator + currentItem.total_value, 0);
...
export function GrossProfit(data:any,revenue:number){
...
export function NetProfitMargin(revenue:number,expense:number){
    return (((revenue - expense) / revenue) * 100).toFixed(1);
}

//Working Capital Ratio 1 decimal digit
export function WorkingCapitalRatio(data:any){
    //Assets calculation
    ...
    const totalAssetDebit = assetDebit.reduce((accumulator: number, item: any) => 
            accumulator + item.total_value, 0);
